...
if [ $stage -le 1 ]; then
    echo "Train PLDA on Voxceleb2"
    steps_be/train_be_plda.sh \
      --cmd "$train_cmd" \
      --lda_dim $lda_dim \
      --plda_type $plda_type \
      --y_dim $plda_y_dim --z_dim $plda_z_dim \
      exp/xvectors/fbank80_stmn_ecapatdnn512x3.adc24.so.s1/adi17/train/xvector.scp \
      data/adi17/train \
      exp/be/fbank80_stmn_ecapatdnn512x3.adc24.so.plda.s1
...
# 			--score-file ${score_dir}adi17__${name}_cores_so.tsv
# 	done
# fi